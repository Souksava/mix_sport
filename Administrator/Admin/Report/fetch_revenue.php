...
{
   $obj->report_revenue_limit(trim($_POST["date1"]),trim($_POST["date2"]),$page);
}
else
{
   $obj->report_revenue_limit("","",$page);
}
$amount = 0;
if(mysqli_num_rows($result_revenue_limit) > 0)
{
 $output .= '
  <div class="table-responsive">
  <table class="table-bordered" style="width: 1800px;text-align: center;" id="tblemplyee">
 ...
 ';
 $no_ =  $rank;
 while($row = mysqli_fetch_array($result_revenue_limit))
 {
...
$no_ += 1;
 ...
 mysqli_free_result($result_revenue_limit);  
 ...
 if(isset($_POST["date1"]) || isset($_POST["date2"]))
{
   $obj->report_revenue(trim($_POST["date1"]),trim($_POST["date2"]));
}
 else
 {
    $obj->report_revenue("","");
 ...
 $count = mysqli_num_rows($result_revenue);
