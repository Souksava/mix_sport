 ...
 if(isset($_POST["query"]))
{
   ...
   $obj->select_broke_limit("%".$_POST['query']."%",$page);
}
else
{
   $obj->select_broke_limit("%%",$page);
}

if(mysqli_num_rows($resultbroke_limit) > 0)
{
 $output .= '
 ...
 ';
 $no_ =  $rank;
 while($row = mysqli_fetch_array($resultbroke_limit))
 {
$no_ += 1;
  $output .= '
    <tr>
        <td>
        <input type="checkbox" name="id2[]" value="'.$row["boke_id"].'" id="flexCheckDefault">
        &nbsp; &nbsp;
        ...
        <td style="display: none;">'.$row["boke_id"].'</td>
        <td>'.$no_.'</td>
        <td>'.$row["pro_id"].'</td>
 ...
 mysqli_free_result($resultbroke_limit);  
 ...
 $count = mysqli_num_rows($resultbroke);
 mysqli_free_result($resultbroke);  
 mysqli_next_result($conn);
 $a = ceil($count/50);
 if(isset($_POST['page'])){
    if($_POST['page'] > 1){
