...
{
   $obj->report_reserv_limit(trim($_POST["date1"]),trim($_POST["date2"]),$page);
}
else
{
   $obj->report_reserv_limit("","",$page);
}
$amount = 0;
if(mysqli_num_rows($result_report_reserv_limit) > 0)
{
 $output .= '
 ...
 ';
 $no_ =  $rank;
 while($row = mysqli_fetch_array($result_report_reserv_limit))
 {
...
$no_ += 1;
 ...
 mysqli_free_result($result_report_reserv_limit);  
 ...
 if(isset($_POST["date1"]) || isset($_POST["date2"]))
{
   $obj->report_reserv(trim($_POST["date1"]),trim($_POST["date2"]));
}
 else
 {
    $obj->report_reserv("","");
 ...
 $count = mysqli_num_rows($result_reserv_sell);
 mysqli_free_result($result_reserv_sell);  
 mysqli_next_result($conn);
 $a = ceil($count/50);
 if(isset($_POST['page'])){
    if($_POST['page'] > 1){
