...
?>
<!-- del one -->
<form action="Unit" id="form_Delete" method="POST">
...
<!-- End del one -->
<!-- Update -->
<form action="Unit" id="form_Update" method="POST">
                ...
                <div class="modal-body">
                    <div class="row" align="left">
                        <div class="col-md-12 col-sm-6 form-control2">
                            ...
                            <input type="text" name="unit_name2" id="unit_name2" placeholder="ຊື່ຫົວໜ່ວຍ"
                                class="form-control">
...
<!-- End Update -->
<!-- Add -->
<form action="Unit" id="form_Save" method="POST">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="row" align="left">
                        <div class="col-md-12 col-sm-6 form-control2">
                            <label>ຊື່ຫົວໜ່ວຍ</label>
                            <input type="text" name="unit_name" id="unit_name" placeholder="ຊື່ຫົວໜ່ວຍ"
                                class="form-control">
                            <i class="fas fa-check-circle "></i>
                            <i class="fas fa-exclamation-circle "></i>
                            <small class="">Error message</small>
...
<form action="Unit" id="form_delete_manay" method="POST">
...
<?php
    ...
    if(isset($_POST["unit_id"])){
        $obj->update_unit($_POST["unit_id"],trim($_POST["unit_name2"]));
    }
    if(isset($_POST["id"])){
        $obj->del_unit($_POST["id"]);
    }
    if(isset($_POST["btnDelete_many"])){
        $logic = 0;
            if(isset($_POST["id2"])){
                foreach($_POST["id2"] as $checkid){
                    $Check_Product = mysqli_query($conn,"call check_unit($checkid)");
                ...
                if($logic == 0){
                    ...
                    foreach($_POST["id2"] as $id){
                        $delete_many = mysqli_query($conn,"call del_unit($id)");
                        if(!$delete_many){
                            $delete = 1;
                            echo"<script>";
...
function checkInputs_form_Save() {
    ...
    if (unit_nameValue !== "") {
        ...
        document.getElementById("form_Save").action = "Unit";
...
function checkInputs_form_Delete() {
    ...
    document.getElementById("form_Delete").action = "Unit";
    document.getElementById("form_Delete").submit();
}
const myform_form_Update = document.getElementById("form_Update");
const unit_name2 = document.getElementById("unit_name2");
...
function checkInputs_form_Update() {
    const unit_name2Value = unit_name2.value.trim();
    if (unit_name2Value === "") {
        setErrorFor(unit_name2, 'ກະລຸນາປ້ອນຊື່ປະເພດສິນຄ້າ');
    } else {
        setSuccessFor(unit_name2);
    }
    if (unit_name2Value !== "") {
        setloading(load_Update, btnUpdate_load);
        document.getElementById("form_Update").action = "Unit";
