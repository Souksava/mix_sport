 ...
 if(isset($_POST["query"]))
{
   ...
   $obj->select_category_limit("%".$_POST['query']."%",$page);
}
else
{
   $obj->select_category_limit("%%", "%%",$page);
}

if(mysqli_num_rows($resultcategory_limit) > 0)
{
 $output .= '
 ...
 ';
 $no_ =  $rank;
 while($row = mysqli_fetch_array($resultcategory_limit))
 {
$no_ += 1;
  $output .= '
    <tr>
        <td>
            <input class="form-check-input" type="checkbox" name="id2[]" value="'.$row["cate_id"].'" id="flexCheckDefault'.$row["cate_id"].'">
            &nbsp; &nbsp;
 ...
 mysqli_free_result($resultcategory_limit);  
 ...
 if(isset($_POST["query"]))
 {
    $obj->select_category("%".$_POST['query']."%");
 }
 else
 {
    $obj->select_category("%%");
